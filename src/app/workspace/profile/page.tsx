...
export default async function ProfilePage() {
  ...
  // Fetch all chapters across user's courses
  const allChapters = courses.length > 0
    ? (await Promise.all(
        courses.map(c =>
          db.select().from(chaptersContentTable).where(eq(chaptersContentTable.cid, c.cid))
        )
      )).flat()
    : []
  ...
  const completedModules = allChapters.filter(c => c.isCompleted).length
  const completedCourses = courses.filter(course => {
    const chapters = allChapters.filter(c => c.cid === course.cid)
    return chapters.length > 0 && chapters.every(c => c.isCompleted)
  }).length
  const overallProgress = totalModules > 0
    ? Math.round((completedModules / totalModules) * 100)
    : 0

  // Recent 3 courses with per-course progress
  const recentCourses = [...courses].reverse().slice(0, 3).map(course => {
    const chapters = allChapters.filter(c => c.cid === course.cid)
    const done     = chapters.filter(c => c.isCompleted).length
    return {
      ...course,
      progress:      chapters.length > 0 ? Math.round((done / chapters.length) * 100) : 0,
      chaptersDone:  done,
    ...
    <div className="min-h-screen bg-[#070708] p-6">
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard icon={Trophy}    label="Completed"        value={completedCourses} color="amber"   />
            ...
            {recentCourses.length === 0 ? (
              <div className="flex flex-col items-center py-10 text-center">
                ...
                <Link href="/workspace/generate"
                  className="mt-3 text-[0.78rem] text-violet-400 hover:text-violet-300 transition-colors">
              ...
              <div className="space-y-3">
                {recentCourses.map(course => (
                  <Link key={course.cid} href={`/workspace/edit-course/${course.cid}`}
                ...
                {courses.length > 3 && (
                  <Link href="/workspace/my-courses"
                    className="block pt-2 text-center text-[0.75rem] text-violet-400/60 hover:text-violet-400 transition-colors">
        ...
        {/* Danger zone */}
        {/* <div className="rounded-2xl border border-red-500/[0.12] bg-red-500/[0.03] p-6">
          <h3 className="mb-1 font-display text-[0.9rem] font-bold text-red-400">Danger Zone</h3>
          <p className="mb-4 text-[0.8rem] text-white/25">
            Permanently delete your account and all associated course data. This cannot be undone.
          </p>
          <button className="rounded-xl border border-red-500/30 px-4 py-2 text-[0.8rem] text-red-400/70 transition-all hover:border-red-500/60 hover:text-red-400">
            Delete Account
          </button>
        </div> */}
